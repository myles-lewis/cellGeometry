...
achieved using the function `updateMarkers()`. Or bulk data can be supplied with
the first call to `cellMarkers()`.
...
new settings, e.g. to alter the number of genes used per subclass or edit
(add/remove) the marker genes. If some signature genes are missing from the bulk
data, `deconvolute()` will stop with an error message that some signature genes
are missing.
...
----------------------  --------------------------------------------------------
`use_filter = TRUE`     This reduces the overall amount of spillover by reducing 
                        apparent noise in the gene signatures.

`convert_bulk = TRUE`   This converts the bulk RNA-Seq onto scRNA-Seq data based 
                        on a reference dataset in which both bulk and scRNA-Seq 
                        were performed simultaneously.
                      
`exp_signature = FALSE` This toggles whether the deconvolution is performed in 
                        count space (exponential space) or in log2 space.
----------------------  --------------------------------------------------------
...
`convert_bulk = TRUE` or `exp_signature = TRUE`, but not both.
