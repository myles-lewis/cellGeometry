...
commands. First, `generate_samples()` makes 25 samples with random cell counts,
`sim_counts`.
...
Then the original scRNA-Seq count dataset is sampled. The actual cell counts in
`sim_counts` can be oversampled by setting `times`.  The desired cell counts are
simply multiplied by `times` prior to sampling. Cells are sampled with
replacement. By default, sampling is performed using the Dirichlet distribution
as this gives true random sampling in comparison to uniform sampling. When cells
are oversampled uniformly, in the limit the summed gene expression tends to the
arithmetic mean of the subclass x sample frequency (which is a much easier
deconvolution problem compared to Dirichlet sampling). This can be demonstrated
by using uniform sampling and steadily increasing `times` from 1 to 30-100 or
more. This improves the deconvolution as the sum of the gene counts per sampled
cell approaches the arithmetic mean of gene counts for each cell cluster.
...
Deconvolution itself is performed using the function `deconvolute()`. The
`plot_set()` function can be used to plot the results. This generates multiple
plots over 1 or more pages, with a scatter plot for each cell subclass. An
alternative way to visualise the results is to use `plot_pred()` which generates
a single scatter plot, with each cell subclass given a different colour. The
`metric_set()` function generates a table of results.
...
in the example below are applied constantly via the `...` system.
...
low expression filter `expfilter` becomes increasingly important and that in
some datasets there is a maximum limit to `nsubclass`.

# Deconvolution diagnostics

## Standard error

Several methods are available to calculate standard errors on a fitted
deconvolution model which can be accessed using the `se()` function. These give
an estimate of the uncertainty/reliability of deconvolution. Note that the
default method which we show to the most accurate gives a vector of SE estimates
with an SE value for each cell subclass but across all bulk samples. The other
methods which are based on ordinary least squares or extensions such as
heteroscedasticity-consistent standard errors using methods HC0, HC2 and HC3,
produce a matrix of SE estimates per subclass and per sample, but these are
considerably noisier and in our view less reliable.
...
Related to this, the condition number of the spillover matrix from a
deconvolution model can be accessed using `kappa()`. In our experience, kappa
tends to be very high with sc-to-bulk deconvolution, because the input data,
which is count data, is on an exponential scale and extremely skewed. Also, most
users actively want to try to deconvolute cell types that are related e.g.
subsets of neurons, subsets of B/T-cells etc. This means that the single-cell
reference gene signature has a strong tendency to collinearity.

## Confidence intervals

Based on the standard error calculation, confidence intervals (default level
95%) can be calculated for all samples using `confint()`.
...
Residual gene expression values can be accessed using `residuals()`. Residuals
are calculated based on the gene signature matrix and stored within the model
object. A full table of residuals for all genes can be produced by providing the
original bulk (or pseudobulk) matrix.
...
Residuals can be visualised with the function `plot_residuals()`. This shows
residuals across all bulk samples on the *y* axis with bulk gene expression
along the *x* axis. Residuals can either be plotted with or without the effect
of equal gene weighting, or as Studentized residuals. A ggplot2 version is
available via `ggplot_residuals()`.
...
Functions that can help users decide if genes are outliers include standardized
or Studentized residuals which can be computed using `rstandard()` and
`rstudent()` respectively. Cook's distance can also be calculated using
`cooks.distance()`.
...
## Multipass outlying gene removal

Genes which are outliers based on extreme residual gene expression are flagged
automatically by the `deconvolute()` function and users alerted with a message
`"Detected genes with extreme residuals ..."`. These can be removed by
successive passes of deconvolution by setting the argument `npass` to >= 2. This
is generally unnecessary in simulations unless large amounts of noise have been
added to the pseudo-bulk. But it may be necessary with real-world data,
especially if cell types which are present in the bulk data are missing from the
reference scRNA-Seq. The multi-pass system stops automatically if no more
outliers can be detected. However, if thresholds for determining outliers are
set too low, the process can go on forever.
...
cellGeometry autotunes compensation for each cell subclass independently as part
of the mechanism to prevent negative cell counts in the deconvolution results.
The package includes functions to visualise the effects of varying compensation.
To enable these, the argument `comp_check` must be set to `TRUE` when calling
`deconvolute()`. With simple simulations, the compensation values are all very
close to 1. However, this system is more deeply invoked and necessary with
real-world bulk samples.
...
Lau R, Cubuk C, Spiliopoulou A, Martinez-Paz P, Surace AEA, Fossati-Jimack L,
Raychaudhuri S, Pitzalis C, Lewis MJ (2026). cellGeometry: ultra-fast
single-cell deconvolution of bulk RNA-Seq using a geometric solution. *bioRxiv*.
