...
In the 2nd mode, the original scRNA-Seq count dataset is sampled. It is possible
to increase the sampling rate of the actual cell counts in `sim_counts` by 3x by
setting `times = 3`. Cells are sampled with replacement. The desired cell counts
are simply multiplied by `times` prior to sampling. By default, sampling is
performed using the Dirichlet distribution as this gives better random sampling
than uniform sampling. When cells are oversampled uniformly, in the limit the
summed gene expression tends to the arithmetic mean of the subclass x sample
frequency. This can easily be tested by using uniform sampling and steadily
increasing `times` from 1 to 30-100 or more. This improves the deconvolution as
the sum of the gene counts per sampled cell approaches the arithmetic mean of
gene counts for each cell cluster.
...
`expfilter` for this single cell dataset seems to be optimal at around 0.25. Of
note, the higher `nsubclass` the better. With deconvolution best with 500-1000
genes per subclass. In reality we find that this is unlikely to be true for
real-world bulk RNA-Seq as noise caused by differences in sequencing chemistry
mean that larger numbers of genes in the signature add increasing noise, so
there is likely to be a sweetspot. Default `nsubclass` is 25 which seems a
