...
#' of expression level.
#' An alternative system of adding noise is invoked if `type = 2`. Here counts
...
#' genes with increasing expression. `sd` is scaled to a mean library size
#' (total counts in each sample/column) of 1e9 counts to adjust for the effect
#' of sampling more cells in [simulate_bulk()] by increasing `times`.
#' 
...
#'   columns.
#' @export
add_noise <- function(counts, sd = 100, sd2 = 0.05, type = 1) {
  sc <- mean(colSums(counts)) / 1e9
  sd <- sd * sc
  if (type == 1) {
    if (sd2 > 0) {
      # Gaussian noise on log scale
      rn <- rnorm(prod(dim(counts)), sd = sd2)
