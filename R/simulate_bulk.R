...
#' be combined. `sd` controls addition of simple Gaussian noise to counts using
#' `rnorm` with sd specified by `sd`. If `sd2` is >0, counts are converted
#' using log2+1 and Gaussian noise added, followed by conversion back to count
#' scale. Negative values are converted to 0. `sd` affects low expressed genes
#' and hardly affects high expressed genes. `sd2` affects all genes irrespective
#' of expression level.
#' A third system of adding noise is invoked if `type = 2`. Here counts are
#' square root transformed before Gaussian noise is added. This still has a
#' stronger effect on low expressed genes, but the effect is more graduated with
#' a more gradual fall off in effect on genes with increasing expression.
#' 
#' @param sim_pseudo An integer count matrix with genes in rows and cell
#'   subclasses typically generated by [simulate_bulk()].
#' @param sd Standard deviation of noise added to counts.
#' @param sd2 Standard deviation of noise added to log2(counts+1). Only applied
#'   if `type = 1`.
#' @param type Either `1` for Gaussian noise on counts and/or log2 counts. Or
#'   `2` in which case Gaussian noise is applied to square root transformed
#'   counts.
#' @returns An integer count matrix with genes in rows and cell subclasses in
#'   columns.
#' @export
add_noise <- function(sim_pseudo, sd = 100, sd2 = 0.05, type = 1) {
  if (type == 1) {
    if (sd2 > 0) {
