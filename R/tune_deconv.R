...
#' @param group Character value specifying column in `res` to be grouped by
#'   colour.
#' @param xvar Character value specifying column in `res` to vary along the x
#'   axis.
...
#' @export
plot_tune <- function(res, group = "subclass", xvar = colnames(res)[1],
                      title = "") {
  ggplot(res, aes(x = .data[[xvar]], y = .data$Rsq, color = .data[[group]])) +
    geom_line() +
    geom_point() +
